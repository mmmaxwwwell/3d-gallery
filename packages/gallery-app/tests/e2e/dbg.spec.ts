...
test.only('plate editor is 3D-first, edits through the rail, and routes by URL', async ({ page }) => {
  test.setTimeout(180_000);

  await page.addInitScript(() => {
    const orig = history.pushState.bind(history);
    history.pushState = function (st: any, t: any, u: any) {
      console.log('PUSH ' + String(u) + ' <- ' + (new Error().stack || '').split('\n').slice(1, 6).join(' | '));
      return orig(st, t, u);
    };
  });
  page.on('console', (m) => { if (m.text().startsWith('PUSH ')) console.log(m.text()); });
  await page.goto('/');
  ...
  console.log('DIRTY MARKER:', await page.locator('.pd-dirty').count());
  await page.goBack();
  await page.waitForTimeout(1500);
  console.log('URL AFTER BACK:', page.url());
  console.log('STAGE COUNT:', await page.locator('.pd-stage').count());
  await expect(page.locator('.pd-stage')).toHaveCount(0);
